...
export default function Navigation() {
    ...
    const routes = [
        { path: '/', name: 'Home' }, // Home link added back
    ...
    return (
        <>
            ...
            {/* Only show navigation menu on non-home pages */}
            {!isHome && (
                <>
                    ...
                    {/* Navigation Panel */}
                    <div className={`fixed top-0 right-0 h-full w-64 bg-gradient-to-br from-white to-violet-200 dark:bg-gradient-to-br dark:from-gray-800 dark:to-violet-900 shadow-lg transform transition-transform duration-300 ease-in-out z-40 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>                        <div className="flex flex-col space-y-4 p-6 mt-16">
                        {routes.map((route) => (
                            <Link
                                key={route.path}
                                href={route.path}
